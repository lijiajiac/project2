            ...
            for (var a = 0; a < files.length; a++) {  //遍历出每个文件对象
                ...
                if (trs.length > 0) {   //判断视频是否已经存在
                    for (var a = 0; a < trs.length; a++) {
                        var na = trs[a].childNodes[1].innerText;
                        var si = trs[a].childNodes[3].innerText;
                        // console.log(na+"--"+si+":"+name+"--"+str);
                        if (si == str && na == name) {
                            alert("请不要上传已上传的视频！！！");
                            return false;
                        }
                        ...
                        for (var b = 0; b < tds.length; b++) {
                            if (tds[b].childNodes.length > 1) {
                                ...
                                sp6.onclick = function () {  //给创建的标签添加点击事件
                                        ...
                                        for (var a = 0; a < sp.length; a++) {
                                            ...
                                            if (na == nod.childNodes[0].innerText && ss == nod.childNodes[1].innerText) {
                                                ...
                                                if (number == 0) {
                                                    ...
                                                    document.getElementById("spotImages").value = s1;     //更新数据库保存的配图信息
                                                } else if (number > 0) {
                                    ...
                                    var coun = 0;
                                    for (var a = 0; a < trs.length; a++) {
                                        var td = trs[a].childNodes[5].childNodes.length;
                                        if (td > 1) {
                                            coun++;
                                        }
                                    }
                                    if (coun == 0) {   //点击后当页面没有未上传的视频才更新文件大小和个数，有则不更新
                                        ...
                                        for (var a = 0; a < trs2.length; a++) {
                                            var chis = trs2[a].childNodes;
                                            if (chis.length > 4) {
                                                var tdsi = chis[3].innerText;
                                                var split = tdsi.split("M");
                                                size = Number(size) + Number(split[0]);
                                            } else {
                                                var tdsi = chis[1].innerText;
                                                var split = tdsi.split("M");
                                                size = Number(size) + Number(split[0]);
                                            }
                                        }
...
//上传后更新大小
    function addtd() {
        ...
        for (var a = 0; a < eles.length; a++) {
            var tds = eles[a].childNodes;
            if (tds.length > 4) {
                var td = tds[3].innerText;
                var tdsize = td.split("M");
                size = Number(size) + Number(tdsize[0]);
            } else {
                var td2 = tds[1].innerText;
                var tdsize = td2.split("M");
                size = Number(size) + Number(tdsize[0]);
            }
        }
